/**
 * The two agent runtimes' brand marks.
 *
 * Both are copied verbatim from `src/renderer/components/agent-provider-brand-marks.tsx`
 * in the app repo (ensemblr-hq/ensemblr), which is where they were first traced
 * and where their provenance is recorded — the same rule as every other fact on
 * this site. Re-copy from there rather than hand-editing a path here: a brand
 * mark redrawn by eye is a vendor's trademark rendered wrong.
 *
 * Both fill with `currentColor`, so the caller decides what colour a mark takes
 * and neither arrives with a fixed one.
 */

// Claude's star mark: the `logos:claude-icon` art from `@iconify-json/logos`,
// re-cut to fill with currentColor. Its source viewBox is 256×257 rather than
// the 24×24 most brand marks ship at.
const CLAUDE_PATH =
...
// Pi's blocky "Pi" mark, traced from https://pi.dev/logo-auto.svg. Its 800×800
// canvas carries a far wider margin than other marks, so the viewBox is cropped
// back to the glyph plus a matching inset — at the source framing Pi renders
